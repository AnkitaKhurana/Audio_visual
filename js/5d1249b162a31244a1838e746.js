...
function ndSoundcloud(args) {
  ...
  this.resolvedEvent = new CustomEvent("ndSoundcloudEvent", { data: {} });
...
ndSoundcloud.prototype = {
  ...
  subscribe : function() {
     ...
     * ndSoundcloudEvent
     */
    var ai$ = Rx.Observable.fromEvent(document.body, 'ndAudioInputEvent');

    // Subscribe to the ndAudioInputEvent stream
    ai$.subscribe(e => {
      this.loadTrack({ trackURL : e.data });
    });
  },
    ...
    this.sdk.get(
      ...
      function(track) {
        ...
        if (typeof track.errors === 'undefined') {
          ...
          this.resolvedEvent.data = this.trackURL;

          // Fire custom event
          document.body.dispatchEvent(this.resolvedEvent);
